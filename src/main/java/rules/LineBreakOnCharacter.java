...
public class LineBreakOnCharacter extends Rule{
    private List<Character> punctuation;
    public LineBreakOnCharacter(int priority, char...punctuation) {
        super(priority);
    ...
    public TranscriptWord applyRule(SRTEntity srtEntity, TranscriptWord word) {
        ...
        for(Character c : punctuation){
            if(c == lastChar){
                ...
                word = TranscriptWord.builder()
                        .from(word).text(word.getText() + "\n").build();
                return word;
            }
        }
        return word;
